...
class CreatureEvent : public Event
{
	public:
		...
		uint32_t executeChannelOpen(Player* player, uint16_t channelId);		
        ...
        uint32_t executeMatou(Creature* creature, Creature* target);
		...
		bool executeTalk(Player* player, const std::string& text, uint16_t channelid); // onTalk
		//
