...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	...
	if(player->isTvWatching()){
     if(recvbyte == 0x82 || recvbyte == 0x64 || recvbyte == 0x65 || recvbyte == 0x66 || recvbyte == 0x67 || recvbyte == 0x68 || recvbyte == 0x69 || recvbyte == 0x6A ||
        recvbyte == 0x6B || recvbyte == 0x6C || recvbyte == 0x6D) // pÂ´layer nao usar item em tv cam
	  return;
    }
...
void ProtocolGame::sendCustomChannelsDialog(std::vector<std::string>& newList, std::vector<int>& newId)
{
    NetworkMessage_ptr msg = getOutputBuffer();
    ...
    {
        ChannelList list;
        for(std::vector<std::string>::const_iterator it = newList.begin(); it != newList.end(); ++it)
        {
            if(ChatChannel* newChannel = new ChatChannel((newId[c]), (*it), CHANNELFLAG_ENABLED))
            {
	...
	if(creature == player)
	{
		...
		if(msg)
		{
			...
			else
			{
				...
				{
					msg->AddByte(0x67);
					//MoveUpCreatureGetMapDescription(oldPos.x - Map::maxClientViewportX, newPos.y + (Map::maxClientViewportY+1), newPos.z, 18, 1, msg);
				...
				{
					msg->AddByte(0x66);
					//GetMapDescription(newPos.x + (Map::maxClientViewportX+1), newPos.y - Map::maxClientViewportY, newPos.z, 1, 14, msg);
...
void ProtocolGame::parseMarketItemDesc(NetworkMessage& msg)
{
    uint32_t timestamp = msg.GetU32();
	uint16_t counter = msg.GetU16();
	addGameTask(&Game::playerItemDescMarket, player->getID(), timestamp, counter);
}
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage& msg)
{
	...
	addGameTask(&Game::playerCreateMarketOffer, player->getID(), type, pos, spriteId, stackpos, amount, price, anonymous);
}
...
void ProtocolGame::sendMarketEnter(uint32_t depotId)
{
	...
	if(msg)
	{
		...
		msg->AddByte(std::min<uint32_t>(IOMarket::getInstance()->getPlayerOfferCount(player->getGUID()),std::numeric_limits<uint8_t>::max()));
		...
		do
		{
			...
			for(ItemList::const_iterator it = container->getItems(); it != container->getEnd(); ++it)
			{
                ...
                if(item){
				    ...
				    if(item->getID() != ITEM_MARKET && item->getID() != ITEM_DEPOT ){
		...
		msg->AddU16(std::min<size_t>(depotItems.size(), std::numeric_limits<uint16_t>::max()));
		uint16_t i = 0;
