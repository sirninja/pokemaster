...
Npc* ScriptEnviroment::getNpcByUID(uint32_t uid)
{
	if(Thing* tmp = getThingByUID(uid))
	{
		if(Creature* creature = tmp->getCreature())
...
void LuaScriptInterface::registerFunctions()
{
    ...
    //getItemSpriteIdById(name[, displayError = true])
	lua_register(m_luaState, "getItemSpriteIdById", LuaScriptInterface::luaGetItemSpriteIdById);
	
	lua_register(m_luaState, "getItemClientIdBySpriteId", luaGetItemClientIdBySpriteId);
	...
	lua_register(m_luaState, "doCreateNpcWithFocus", LuaScriptInterface::luaDoCreateNpcWithFocus);
	...
	//setNpcFocus(cid, Npc)
	lua_register(m_luaState, "setNpcFocus", LuaScriptInterface::luaSetNpcFocus);
	
	lua_register(m_luaState, "isSummon", LuaScriptInterface::luaIsSummon);
	...
	//doPlayerOpenChannelsList(cid, list, count)   
	lua_register(m_luaState, "doPlayerOpenChannelsList", LuaScriptInterface::luaDoPlayerOpenChannelsList);
	...
	lua_register(m_luaState, "getNpcFocus", LuaScriptInterface::luaGetNpcFocus);
	
	lua_register(m_luaState, "getNpcMaster", LuaScriptInterface::luaGetNpcMaster);
    ...
    lua_register(m_luaState, "setNpcFollow", LuaScriptInterface::luaSetNpcFollow);
...
int32_t LuaScriptInterface::luaDoCreateMonsterNick(lua_State* L)
{
	...
	{
		lua_pushboolean(L, false);
		errorEx("Wrong position to create monster");
		return 1;
	}
...
int32_t LuaScriptInterface::luaDoPlayerAddItemEx(lua_State* L)
{
    ...
    if(item->getParent() == VirtualCylinder::virtualCylinder)
        lua_pushnumber(L, g_game.internalPlayerAddItem(NULL, player, item, canDropOnMap, slot));
    else
        lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoPlayerOpenChannelsList(lua_State* L)
{
//doPlayerOpenChannelsList(cid, list, count)   
    
    std::vector<std::string> newList; //inicia um novo vetor
    std::vector<int> newId; //inicia um novo vetor
    uint32_t tam = lua_gettop(L); //define a variavel tam como todos parametros depois do cid
    ScriptEnviroment* env = getEnv();
    
    for(uint32_t i = 0; i < tam-1; ++i){ //faz um for na tabela tam 
        if(i%2 == 0) {//separa os impares dos pares
            newList.push_back(popString(L)); //impar
        } else {
            newId.push_back(popNumber(L)); //par 
        }
    }
    Player* player = env->getPlayerByUID(popNumber(L));
    if(!player)
    ...
    player->sendCustomChannelsDialog(newList, newId); //manda o newList
    lua_pushboolean(L, true);
    return 1;
}
...
int32_t LuaScriptInterface::luaDoCreateNpcWithFocus(lua_State* L)
{
	//doCreateNpcWithFocus(name, pos, focus)
	int32_t pp = popNumber(L);
	PositionEx pos;
	popPosition(L, pos);
	std::string name = popString(L);
	ScriptEnviroment* env = getEnv();
	Npc* npc = Npc::createNpc(name.c_str());
	Player* player = env->getPlayerByUID(pp);
	if(!player)
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoCreatureCastSpell(lua_State* L) // by OrochiElf.
{
    ...
    if(Creature* creature = env->getCreatureByUID(popNumber(L)))
    {
		...
		Npc* npc = env->getNpcByUID(popNumber(L));
		if(npc)
		{
			Creature* target = npc->getAttackedCreature();
...
int32_t LuaScriptInterface::luaSetNpcFocus(lua_State* L)
{
	//setNpcFocus(cid, npc)
	uint32_t nid = popNumber(L);
    ScriptEnviroment* env = getEnv();
	Npc* npc = env->getNpcByUID(nid);
	if(!npc)
	{
...
int32_t LuaScriptInterface::luaGetNpcFocus(lua_State* L)
{
	//setNpcFocus(npc)
	uint32_t nid = popNumber(L);
    ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetNpcMaster(lua_State* L)
{
	//setNpcMaster(npc)
	uint32_t nid = popNumber(L);
    ScriptEnviroment* env = getEnv();
	...
	lua_pushnumber(L, npc->npcMaster);
	return 1;
}
...
int32_t LuaScriptInterface::luaSetNpcFollow(lua_State* L)
{
	uint32_t nid = popNumber(L);
    ScriptEnviroment* env = getEnv();
	Npc* npc = env->getNpcByUID(nid);
	...
	Creature * creature = npc->getFollowCreature();
	if(creature){
		if (player->getID() != creature->getID()){
...
int32_t LuaScriptInterface::luaGetItemSpriteIdById(lua_State* L)
{
	//getItemSpriteIdById(name[, displayError = true])
	bool displayError = true;
	if(lua_gettop(L) >= 2)
	...
	int32_t itemId = Item::items.getItemSpriteIdById(popNumber(L));
	if(itemId == -1)
	{
		if(displayError)
			errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
...
int32_t LuaScriptInterface::luaGetItemClientIdBySpriteId(lua_State* L)
{
	//getItemSpriteIdById(name[, displayError = true])
	bool displayError = true;
	if(lua_gettop(L) >= 2)
	...
	int32_t itemId = Item::items.getItemIdByClientId2(popNumber(L));
	if(itemId == -1)
	{
		if(displayError)
			errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
