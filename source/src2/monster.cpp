...
void Monster::onThink(uint32_t interval)
{
	...
	{
		std::string strValue;
        getMaster()->getStorage(500, strValue);
        if (strValue == "-1") {
			Npc* npc = getMaster()->getNpc();
			if(!attackedCreature)
			{
				if (npc){
					if (npc->getFocus() == 0){
						...
						Creature* tt = g_game.getCreatureByID(npc->getFocus());
						if(followCreature){
						    if (npc->getCanKill() == 0){
							   const std::list<Creature*>& summons = tt->getSummons();
							   CreatureList::const_iterator itt = summons.begin();
							   for(uint32_t i = 1; itt != summons.end(); ++itt, ++i)
							   {
								    if (i >= 1 && *itt){
									    selectTarget(*itt);
									    attackedCreature = *itt;
									    if(followCreature != attackedCreature)
										   setFollowCreature(*itt);
								    }else{
									    if(getMaster() != followCreature)
										    setFollowCreature(getMaster());	
								   }
							    ...
							    attackedCreature = tt;
							    if(followCreature != attackedCreature)
								    setFollowCreature(tt);	
						   }
					    }
					}
				}
				else if(getMaster() && getMaster()->getAttackedCreature()) //This happens if the monster is summoned during combat
...
void Monster::setAttackSpeed(uint32_t interval)
{
...
void Monster::onThinkTarget(uint32_t interval)
{
	...
	if(mType->targetDistance <= 1 && value == -1)
