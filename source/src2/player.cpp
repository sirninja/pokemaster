...
void Player::notifyLogIn(Player* loginPlayer)
{
	...
	if(it != VIPList.end()) {
		...
		std::stringstream ss;
		ss << loginPlayer->getName() << " has logged in.";
		sendCancel(ss.str().c_str());
	}
}
...
void Player::notifyLogOut(Player* logoutPlayer)
{
	...
	if(it != VIPList.end()) {
		...
		std::stringstream ss;
		ss << logoutPlayer->getName() << " has logged out.";
		sendCancel(ss.str().c_str());
	}
}
...
bool Player::hasCapacity(const Item* item, uint32_t count) const
{
	...
	return true;//(itemWeight < getFreeCapacity());
}
...
{
		sendTextMessage(MSG_EVENT_ADVANCE, "Acabou de receber seus itens no market-depot.");
}
