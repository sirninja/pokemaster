...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		std::string getHeldX() const;
		std::string getHeldY() const;
...
{
	const std::string* v = getStringAttribute("poke");
	if(v)
		return *v;
...
{
	const std::string* v = getStringAttribute("ball");
	if(v)
		return *v;
...
inline std::string Item::getNick() const
{
	const std::string* v = getStringAttribute("nick");
	if(v)
		return *v;
...
inline std::string Item::getHeldX() const
{
	const std::string* v = getStringAttribute("xHeldItem");
	if(v)
		return *v;
...
inline std::string Item::getHeldY() const
{
	const std::string* v = getStringAttribute("yHeldItem");
	if(v)
		return *v;
...
inline int32_t Item::getBoost() const
{
	const int32_t* v = getIntegerAttribute("boost");
	if(v)
		return *v;
...
inline uint32_t Item::countByType(const Item* i, int32_t subType){
	if (subType == -1 || subType == (int32_t)i->getSubType()) {
