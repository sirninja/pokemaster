...
class Player : public Creature, public Cylinder
{
	public:
		...
		bool isTvWatching() const {
             std::string value;
             if(getStorage(55017, value) && value == "1")
                return true;
             else
                return false;
       }
		...
		void sendMarketLeave()
			{inMarket = false; if (client) {client->sendMarketLeave();}}
		...
		void sendMarketItemDesc(const std::string& desc, uint16_t itemId) const
			{if(client) client->sendMarketItemDesc(desc, itemId);}
		...
		void sendCustomChannelsDialog(std::vector<std::string>& newList, std::vector<int>& newId)
            {if(client) client->sendCustomChannelsDialog(newList, newId);}
