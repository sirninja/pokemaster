	...
	int32_t subType/* = -1*/, bool addArticle/* = true*/)
{
	...
	bool dot = true;
	std::stringstream s;
	if(item->hasStringAttribute("ball")){
		dot = false;
		std::string ballUpper = item->getBall();
		ballUpper[0] = toupper(ballUpper[0]);
		s << ballUpper << "ball." << std::endl;
		if(item->hasStringAttribute("poke")){
			s << "It contains a " << item->getPoke() << "." << std::endl;
			
			if(item->hasIntegerAttribute("boost"))
				s << "Boost level: +" << item->getBoost() << "." <<std::endl;
			
			if(item->hasStringAttribute("nick"))
				s << "It's nickname is': " << item->getNick() << "." << std::endl;
				
			if(item->hasStringAttribute("xHeldItem") && item->hasStringAttribute("yHeldItem")){
				s << "Holding: " << item->getHeldX() << " and " << item->getHeldY() << "." << std::endl;
			}else{
				if(item->hasStringAttribute("xHeldItem"))
					s << "Holding: " << item->getHeldX() << "." << std::endl;	
				
				if(item->hasStringAttribute("yHeldItem"))
					s << "Holding: " << item->getHeldY() << "." << std::endl;
			}
