...
void Npc::onThink(uint32_t interval)
{
	...
	Creature* cret = getAttackedCreature();
	Creature* follo = getFollowCreature();
	Creature* mestre = g_game.getCreatureByID(npcMaster);
	uint32_t ff = 0;
	if(follo){
		ff = follo->getID();	
	}
	if(cret){
		int32_t lookDistance = std::max(std::abs(getPosition().x - cret->getPosition().x), std::abs(getPosition().y - cret->getPosition().y));
		if (cret->getID() != ff && lookDistance <= 13){
			setFollowCreature(cret, true);
		}
	}else{
		if(mestre)
			setFollowCreature(mestre, true);
	}
	...
	for(StateList::iterator it = stateList.begin(); it != stateList.end();)
	{
		...
		if(!npcState->isQueued)
		{
			...
			else if(canIdle && idleTime > 0 && (OTSYS_TIME() - npcState->prevInteraction) > (uint64_t)(idleTime * 1000))
				...
				case ACTION_BUYITEM:
				{
					...
					if(iit.id != 0)
					{
						...
						if(g_game.getMoney(player) >= moneyCount)
						{
							...
							//player->balance = player->balance - moneyCount; 
						}
					}
					break;
				}
...
void Npc::setCanKill(int32_t num)
{
	canKillPlayer = num;
}

void Npc::setCanIdle(bool bb)
{
	canIdle = bb;
}
...
void NpcScriptInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "selfCanKill", NpcScriptInterface::luaActionCanKill);
	lua_register(m_luaState, "selfCanIdle", NpcScriptInterface::luaActionCanIdle);
	lua_register(m_luaState, "selfSay", NpcScriptInterface::luaActionSay);
	
	lua_register(m_luaState, "getFollow", NpcScriptInterface::luaGetNpcFollow);
	...
	lua_register(m_luaState, "selfAttackedCreature", NpcScriptInterface::luaActionAttackedCreature);
    lua_register(m_luaState, "selfMaster", NpcScriptInterface::luaActionMaster);
...
int32_t NpcScriptInterface::luaActionCanKill(lua_State* L)
{
	//selfCanKill(cid)
	ScriptEnviroment* env = getEnv();
	...
	npc->setCanKill(popNumber(L));
	return 0;
}

int32_t NpcScriptInterface::luaActionCanIdle(lua_State* L)
{
	//selfCanIdle(cid)
	ScriptEnviroment* env = getEnv();
...
int32_t NpcScriptInterface::luaActionFollow(lua_State* L)
{
	...
	Creature* player = env->getCreatureByUID(cid);
	if(cid && !player)
	{
...
int32_t NpcScriptInterface::luaActionAttackedCreature(lua_State* L)
{
	//selfAttackedCreature(cid)
	uint32_t cid = popNumber(L);
	ScriptEnviroment* env = getEnv();
	...
	lua_pushboolean(L, npc->setAttackedCreature(creature));
	return 1;
}

int32_t NpcScriptInterface::luaActionMaster(lua_State* L)
{
	//selfMaster(cid)
	uint32_t cid = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t NpcScriptInterface::luaGetNpcFocus(lua_State* L)
{
	//getNpcFocus()
	ScriptEnviroment* env = getEnv();
	Npc* npc = env->getNpc();
...
int32_t NpcScriptInterface::luaGetNpcFollow(lua_State* L)
{
	//getNpcFollow()
	ScriptEnviroment* env = getEnv();
	Npc* npc = env->getNpc();
