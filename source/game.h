...
class Game
{
	public:
        ...
        ReturnValue placeSummon(Creature* creature, const std::string& name, const std::string& nick);
		...
		ReturnValue internalPlayerAddItem(Creature* actor, Player* player, Item* item, bool dropOnMap = true, uint32_t slot = 0);
		...
		bool playerCreateMarketOffer(uint32_t playerId, uint8_t type, const Position& pos, int16_t stackpos, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous);
		bool playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter);
		bool playerItemDescMarket(uint32_t playerId, uint32_t timestamp, uint16_t counter);
