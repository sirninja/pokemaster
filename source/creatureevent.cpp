...
bool CreatureEvent::configureEvent(xmlNodePtr p)
{
    ...
    else if(tmpStr == "matou")
        m_type = CREATURE_EVENT_MATOU;
	else if(tmpStr == "extendedopcode")
        m_type = CREATURE_EVENT_EXTENDED_OPCODE;
	else if(tmpStr == "talk")
		m_type = CREATURE_EVENT_TALK;
...
std::string CreatureEvent::getScriptEventParams() const
{
	switch(m_type)
	{
        ...
        case CREATURE_EVENT_ONMOVE:
			return "cid, item, fromPosition, toPosition, fromItem, toItem, fromGround, toGround, status";
        case CREATURE_EVENT_MOVE:
...
uint32_t CreatureEvent::executeChannelOpen(Player* player, uint16_t channelId)
{
	...
	{
		std::cout << "[Error - CreatureEvent::executeChannelOpen] Call stack overflow." << std::endl;
		return 0;
	}
}

uint32_t CreatureEvent::executeMatou(Creature* creature, Creature* target)
{
	//matou(cid, target)
	if(m_interface->reserveEnv())
	{
...
bool CreatureEvent::executeTalk(Player* player, const std::string& text, uint16_t channelid)
{
	//onTalk(cid, text, channelId)
	if(m_interface->reserveEnv())
	{
		ScriptEnviroment* env = m_interface->getEnv();
		if(m_scripted == EVENT_SCRIPT_BUFFER)
		...
		else
		{
			...
			lua_pushnumber(L, (uint16_t)channelid);
