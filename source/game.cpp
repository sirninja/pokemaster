	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	if(Creature* movingCreature = thing->getCreature())
	{
		...
		{
			SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerMoveCreature, this, player->getID(), movingCreature->getID(), movingCreature->getPosition(), toCylinder->getPosition()));
    ...
    uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
    ...
    if (!player->hasCustomFlag(PlayerCustomFlag_CanPushAllItems) && (!item->isPushable() || (item->isLoadedFromMap() && (item->getUniqueId() != 0 || (item->getActionId() != 0 && item->getContainer()))))) {
        player->sendCancelMessage(RET_NOTMOVEABLE);
        return false;
    }
	...
	uint32_t flags /*= 0*/, bool test /*= false*/)
{
	...
	if(!hasSpaceInContainer(player->getInventoryItem((slots_t)3))) {
		ret = RET_NOERROR;
	}
...
ReturnValue Game::internalPlayerAddItem(Creature* actor, Player* player, Item* item, bool dropOnMap /*= true*/, uint32_t slot)
{
...
bool Game::playerOpenChannel(uint32_t playerId, uint16_t channelId)
{
	...
	bool deny = false; 
	CreatureEventList openChannelEvents = player->getCreatureEvents(CREATURE_EVENT_CHANNEL_OPEN);	
    for(CreatureEventList::iterator it = openChannelEvents.begin(); it != openChannelEvents.end(); ++it)
    {
        if(!(*it)->executeChannelOpen(player, channelId))
            deny = true;
    }
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
   ...
   if(player->isTvWatching() && (channelId < 100 || channelId > 10000))
	...
	//	onTalk	
	uint16_t channelID = (type == SPEAK_SAY ? -1 : type == SPEAK_PRIVATE ? 0 : channelId);
	
	// channelId: -1 = Default Channel
	// channelId: 0 = Private Channel
	
	bool deny = false;
	CreatureEventList talkEvent = player->getCreatureEvents(CREATURE_EVENT_TALK);
	for(CreatureEventList::iterator it = talkEvent.begin(); it != talkEvent.end(); ++it) {
			deny = !(*it)->executeTalk(player, text, channelID);
	}	
...
bool Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter, uint16_t amount)
{
	...
	else
	{
		...
		if(sellerPlayer)
			addMoney(sellerPlayer->getDepot(2, false), totalPrice, FLAG_NOLIMIT);
		else{
